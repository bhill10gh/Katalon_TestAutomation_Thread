...
class EventFiltersRegion {
	...
	def I_view_the_number_of_results_shown_in_the_Content_Type_dropdown_Region_1_Cnt() {
		...
		WebUI.verifyMatch(text, "Region (1)", false, FailureHandling.STOP_ON_FAILURE)
	}
	...
	@Then("The number of results matches the count shown_1_Cnt")
	def I_verify_the_status_in_step() {
		WebDriver driver = DriverFactory.getWebDriver()
		List<WebElement> elements = driver.findElements(By.className("list-item__content"))
		println("Results Count = " + elements.size())
		if(elements.size() != 1) {
			KeywordUtil.markFailed("Region does not only contain 1 result.")
		}
