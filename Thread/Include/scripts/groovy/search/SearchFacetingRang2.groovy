...
class SearchFacetingRang2 {
	/**
	 * The step definitions below match with Katalon sample Gherkin steps
	 */

	@Given("Open Browser Goto Search Page (.*) for Search Faceting Rang 2")
	...
	@When("Enter search (.*) for Search Faceting Rang 2")
	...
	@Then("Verify Region Facet count is (\\d+) for Search Faceting Rang 2")
	def VerifyFacetRegionCount(int regionFacetCnt) {
		...
		if(intVal  != regionFacetCnt){
			KeywordUtil.markFailed('ERROR: Number of results (' + intVal+ ') does not equal' +  regionFacetCnt)
		}
